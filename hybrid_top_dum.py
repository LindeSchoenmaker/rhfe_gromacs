        ...
        if line.strip() == "":
            ...
            for endstate in ['A', 'B']:
                for i in range(len(bridge_atoms[endstate])):
                    if bridge_atoms[endstate][i][0] in comment and bridge_atoms[endstate][i][1] in comment:
                        if any(ext in comment for ext in P2s[i]):
                            ...
                            # for dual junction use high force constant and angle of 90 degrees
                                ...
                                if endstate == 'A': 
                                    line = ai.rjust(6) + aj.rjust(7) + ak.rjust(6) + funct.rjust(8) + dual_angle.rjust(9) + dual_fc.rjust(16) + c2.rjust(20) + k2.rjust(15) + comment + "\n"
                                elif endstate == 'B':
                                    line = ai.rjust(6) + aj.rjust(7) + ak.rjust(6) + funct.rjust(8) + c1.rjust(15) + k1.rjust(15) + dual_angle.rjust(9) + dual_fc.rjust(16) + comment + "\n"
        ...
        if line.split()[0] == ";":
            ...
            if second.count("A") > 1 and "D" in second:
                if any(ext in comment for ext in P2s):
                    if second.count("A") != 2:
                        if altered:
                            raise AssertionError('Dihedral between two dummy atoms is made')
